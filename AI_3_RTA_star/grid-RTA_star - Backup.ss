...
(define get-next-robot 
  (lambda (point)
    (let* ((lst1 (cons point (adjacento point)))  ;; get adjacent points of the current position
      ...
      (set! queue '()) ;; clear the queue
      (enqueue lst)    ;; enqueue the new list
      (display "move: ") (display (front)) (newline)
      (front)))) ;; return the best move (front of the queue)
...
(define search-robot-old
  (lambda (grid)
    (let ((next-robot (get-next-robot robot))) ;; get the next move for the robot
      ...
      ; if the move i same as previous, move to the second best option
      (if (equal? next-robot robot)
        (begin
        ...
        (begin
          ...
          (if (not (null? robot))
            (draw-moved-robotx robot))))))) ;; move the robot
