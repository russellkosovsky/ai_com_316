...
(define draw-row
  (lambda (row col lst)
    (cond
      ...
      ((= (car lst) obstacle-stable-high)
        (send canvas make-obstacle-stable-high col row)
        (draw-row row (+ col 1) (cdr lst)))
      ((= (car lst) obstacle-stable-low)
        (send canvas make-obstacle-stable-low col row)
        (draw-row row (+ col 1) (cdr lst)))
      ((= (car lst) obstacle-unstable-high)
        (send canvas make-obstacle-unstable-high col row)
        (draw-row row (+ col 1) (cdr lst)))
      ((= (car lst) obstacle-unstable-low)
        (send canvas make-obstacle-unstable-low col row)
        (draw-row row (+ col 1) (cdr lst)))
      (else
