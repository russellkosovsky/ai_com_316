  ...
  (lambda (point)
    (define minimax-inner
      (lambda (current-list current-depth)
        (cond ((= current-depth max-depth) (set! backtrack #t)))
        (cond ((eq? backtrack #t)
            (cond
              ((even? current-depth)
                (let ((sorted-list (list-sort heuristic-compare-min current-list)))
                  (car sorted-list)))
              (else
                (let ((sorted-list (list-sort heuristic-compare-max current-list)))
                  (car sorted-list)))))
          ;; Recursive case: expand and recurse
          (else
            (let ((next-results (map (lambda (x)
                                       (minimax-inner (expand-max x) (+ current-depth 1)))
                                     current-list)))
              ;; Display next-results directly
              ...
              (newline)
              next-results)))))
    ;; Start minimax from the root point
